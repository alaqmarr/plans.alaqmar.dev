...
import { togglePlanFeature } from "@/app/actions/features";
...
export default function EditPlanClient({ plan, allFeatures }: { plan: any, allFeatures: any[] }) {
  ...
  const [paymentTerms, setPaymentTerms] = useState(() => {
    try {
      return JSON.parse(plan.paymentTerms);
    } catch {
      return [
  ...
  const [formData, setFormData] = useState({
    ...
    offerPrice: plan.discountPrice ? plan.discountPrice.toString() : plan.price.toString(),
    tenureYears: plan.tenureYears || 1,
  ...
  const estimatedBaseCost = allFeatures
    .filter((f) => plan.features.some((pf: any) => pf.featureId === f.id && pf.isIncluded))
    .reduce((sum, f) => {
      const fp = f.price || 0;
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8">
      <div className="flex items-center gap-4">
        ...
        <h1 className="font-outfit text-4xl font-extrabold tracking-tight text-white">Edit Plan: {plan.name}</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-3xl p-8 relative shadow-2xl overflow-hidden">
          ...
          <form id="edit-plan-form" onSubmit={handleSubmit} className="space-y-6 relative z-10">
            <div className="space-y-5">
              ...
              <div className="pt-8 border-t border-white/5">
                ...
                <div className="space-y-4">
                  {paymentTerms.map((term: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-4">
                      ...
                      <button
                        type="button"
                        onClick={() => setPaymentTerms(paymentTerms.filter((_: any, i: number) => i !== idx))}
                  ...
                  {(() => {
                    const total = paymentTerms.reduce((sum: number, t: any) => sum + (t.percent || 0), 0);
                    return (
        ...
        <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-3xl p-8 shadow-2xl overflow-hidden self-start sticky top-12">
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {allFeatures.map((feature) => {
              const isIncluded = plan.features.some((pf: any) => pf.featureId === feature.id && pf.isIncluded);
              return (
                <button
                  key={feature.id}
                  onClick={() => handleToggleFeature(feature.id, isIncluded)}
                  type="button"
